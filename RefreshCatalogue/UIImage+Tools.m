...
+ (CGFloat) detectSkewOfTheImage:(UIImage *) image withDegreesRangeMinimum:(CGFloat) degMinimum andDegreesMaximum:(CGFloat) degMaximum degreesStep:(CGFloat) degStep { 
	...
	for(; currentCheckingAngleInPixels < maximumCheckingAngleInPixels && currentCheckingAngleInPixels < image.size.height; currentCheckingAngleInPixels += stepAngle){
		int blackPercentageForGivenAngle = (int)[UIImage getBlackPercentageForAngle:currentCheckingAngleInPixels forImageData:image andBlackTreshold:blackTreshold];
...
- (CGFloat)scaleOfAnImageScaledToFill:(CGSize)size {
    CGFloat scale = (((self.size.width * 100.0f) / size.width) / 100);
    return scale;
}

- (CGSize)sizeOfAnImageScaledToFill:(CGSize)size {
    CGSize s = self.size;
...
- (CGSize)realSizeOfAnImageScaledToFill:(CGSize)size {
    CGFloat scale = [self scaleOfAnImageScaledToFill:size];
